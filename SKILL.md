...
Turn Figma designs into faithful, responsive, maintainable WordPress custom themes that editors can use confidently. Figma is the authoritative source of truth for visual design and assets. For new themes, prefer Timber v2 + Twig + ACF with structured page fields; also support classic PHP themes when requested. Preserve an existing project's stack unless a migration is requested.
...
Figma is mandatory as the source of truth for visual assets and design decisions. Use the exact assets, text, icons, separators, illustrations, logos, typography, colors, effects, and variants from the referenced Figma file. A user-provided export is acceptable only when it is identified as an exact Figma export; screenshots and written briefs can supplement geometry or content context but do not authorize recreating or substituting assets. Without direct Figma access, state the evidence available, do not claim node inspection, and request access or the exact export before implementing a missing visual asset. Never fill an evidence gap with a Unicode glyph, CSS-drawn approximation, generic icon, invented SVG, AI-generated image, fabricated copy, or guessed spacing. Without WordPress/PHP/browser tools, complete feasible implementation and static review, then distinguish unavailable runtime checks from passed checks. Resolve paths and commands from the current environment; do not assume macOS, a particular local server, or private tooling.
...
2. Treat Figma frames, components, variables, variants, assets, and prototype states as the authoritative source. Use the actual Figma asset or an exact Figma export for every visible design asset; preserve its text, proportions, colors, spacing, and relationships. Do not redesign, embellish, approximate, or replace a supplied design unless the user explicitly requests it. Separate measured evidence from inferred responsive behavior.
3. Establish asset provenance before implementation. For each visible image, icon, separator, logo, illustration, font, or decorative element, identify the Figma node/export used. If the source is missing or inaccessible, preserve the gap and ask for the exact Figma asset instead of inventing a replacement.
4. Default new projects to Timber v2 + Twig + ACF. Interpret “normal method” as classic PHP templates; ACF is optional there. Do not migrate existing themes, install page builders, or switch to block themes implicitly.
5. Prefer native titles, content, excerpts, images, menus, and taxonomies where they fit. Default ACF to structured fields attached to templates; Flexible Content and ACF blocks are deliberate editorial choices, not automatic scaffolding. Keep parent-owned rows in Groups or Repeaters, but use native records or a custom post type plus queries/relationships when items need an independent publication lifecycle or reuse.
6. Check installed versions and official WordPress, Timber, Twig, and ACF documentation for version-sensitive APIs. Do not mix Timber v1 constructors with v2 factory APIs or guess plugin capabilities.
7. Keep data retrieval and normalization in PHP and presentation in Twig/template parts. Avoid repeated queries in components and unnecessary abstraction layers.
8. Preserve WordPress integration hooks, template hierarchy, content filters, plugin compatibility, translation conventions, and contextual output escaping. Do not assume Twig or ACF values are safe HTML.
9. Keep persistent content types and business behavior in the project's plugin layer when available. Do not create or relocate plugins as an incidental theme edit.
10. Check ACF edition and feature availability before relying on paid fields. Do not bundle licenses, secrets, or commercial plugin code into the theme.
11. Match the design with real HTML and responsive layouts. Do not use a full-page screenshot as the implementation or absolute positioning for the entire page.
12. Preserve existing build tools and CSS conventions. Add a build system only when justified; deliver assets using the actual deployment workflow.
13. Scope work to the requested theme and pages. Theme activation, content imports, production deployment, and Figma edits require authorization covering those actions. Existing explicit or clearly implied authorization counts; do not ask again for routine actions already covered by the task.

14. Establish a source-of-truth lock before implementation: record the design file, page, desktop/mobile frames, revision/date, and any explicitly latest variant. Do not mix assets or measurements from older frames without labeling the exception.
15. Separate editorial content from presentation assets. Send copy, content imagery, links, repeated items, and data-driven labels through native WordPress/ACF structures; keep logos, decorative patterns, overlays, icons, type scale, spacing, and layout rules in the theme unless the design explicitly makes them editable.
16. Use an asset contract. Give source assets human-readable semantic names, preserve originals in a non-deployed archive, and commit only referenced web-ready outputs. Validate extensions against file contents, cap dimensions to the largest rendered use, strip unnecessary metadata, use modern formats with an appropriate fallback, and remove unused outputs. Every output must trace back to a Figma asset or exact Figma export; do not create a substitute merely because it is easier to implement.
17. Define a shared layout contract before styling: content max-width, gutters, spacing tokens, typography roles, and the explicit edge-to-edge sections. Reuse those values across sections; document measured exceptions instead of accumulating one-off overrides.
18. Treat missing editorial data as a modeled state. Never invent URLs, claims, production copy, or visual assets to fill a gap. Preserve intentional geometry with a safe fallback, disabled action, or clearly documented placeholder, and make the ACF/native field contract explicit before wiring it.
19. Verify the rendered result against the locked source at its supplied desktop and mobile sizes, then test at representative narrow, tablet, and wide widths. Check overflow, image loading, intrinsic/display dimensions, keyboard states, browser/PHP errors, and the absence of stale, invented, or unused assets before reporting completion.
