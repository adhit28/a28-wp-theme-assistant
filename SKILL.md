...
4. Prefer native titles, content, excerpts, images, menus, and taxonomies where they fit. Default ACF to structured fields attached to templates; Flexible Content and ACF blocks are deliberate editorial choices, not automatic scaffolding. Keep parent-owned rows in Groups or Repeaters, but use native records or a custom post type plus queries/relationships when items need an independent publication lifecycle or reuse.
